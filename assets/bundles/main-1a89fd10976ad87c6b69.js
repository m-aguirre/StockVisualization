...
var _RegressionOutlierDetector = __webpack_require__(28);

var _RegressionOutlierDetector2 = _interopRequireDefault(_RegressionOutlierDetector);

var _dataFile = __webpack_require__(30);

var _dataFile2 = _interopRequireDefault(_dataFile);

var _OutlierDetector = __webpack_require__(31);
...
var App = function (_React$Component) {
    ...
    _createClass(App, [{
        ...
        value: function render() {
            return _react2.default.createElement(_OutlierDetector2.default, null);
        }
    }]);
...
var _DateScale = __webpack_require__(29);
...
var RegressionOutlierDetector = function () {
  ...
  */
  ...
  _createClass(RegressionOutlierDetector, [{
    key: "millisecondDelay",
    value: function millisecondDelay() {
      var ms = 0;
    ...
    */
    ...
    key: "calculateRegressionEquation",
    value: function calculateRegressionEquation(data) {
      var _this = this;
    ...
    key: "identifyOutliers",
    value: function identifyOutliers(data, sigma) {
      var _this2 = this;
    ...
    value: function addViewport() {
      d3.select('.graph-pane').append('svg').attr('class', 'viewport').attr('width', 700).attr('height', 450);
    ...
    value: function plotDataPoints() {
      ...
      for (var i = 0; i < this.data.length; i++) {
        ...
        dots.append("circle").data(data).attr("r", 0).attr("cx", function (d) {
          ...
          d3.select('.viewport').selectAll('rect').remove();
          d3.select('.viewport').selectAll('.outlier-data').remove();
        }).style('stroke', 'black').style('fill', 'white').transition().delay(this.delayFactor * i).attr("r", 3.5);
      }
      setTimeout(function () {
        _this3.drawRegressionLine();
      }, this.millisecondDelay());
      setTimeout(function () {
        _this3.colorOutliersRed(_this3.data);
      }, this.millisecondDelay() + 750);
    }
  }, {
    ...
    value: function colorOutliersRed(data) {
      d3.select('.viewport').selectAll('circle').transition().duration(1200).style('stroke', function (d) {
        return d.outlier ? '#ff0202' : '#bcbcbc';
    ...
    key: "showInfo",
    value: function showInfo(outlier) {
      var cx;
      ...
      var d3ViewPort = d3.select('.viewport');
      var svg = d3ViewPort.append('svg');
      var rect = svg.append('rect').attr('width', 125).attr('height', 55).attr('class', 'outlier-info-box').attr('x', cx).attr('y', outlier.attr('cy')).attr('rx', 5).attr('ry', 5);

      svg.append('text').attr('class', 'outlier-data').attr("dx", function (d) {
        return cx + 10;
      }).attr("dy", function (d) {
        return +outlier.attr('cy') + 20;
      }).text("Date: " + outlier.attr('date'));

      svg.append('text').attr('class', 'outlier-data').attr("dx", function (d) {
        return cx + 10;
      }).attr("dy", function (d) {
        return +outlier.attr('cy') + 42.5;
      }).text("Close: $" + outlier.attr('close').slice(0, 5));
    }
  }]);

  return RegressionOutlierDetector;
}();

exports.default = RegressionOutlierDetector;
